...
async def test_create_user_success(
    ...
    user_id_bytes: bytes = bytes(Ulid(user.id))
    created_user = await motor_database.users.find_one({'_id': user_id_bytes})

    assert created_user is not None
    assert isinstance(created_user, dict)
    assert created_user.get('_id') == user_id_bytes
...
async def test_update_user_success(
    ...
    updated_user = await motor_database.users.find_one(
        {'_id': bytes(Ulid(new_user.id))}
    )

    assert updated_user is not None
    assert isinstance(updated_user, dict)
    assert str(Ulid(updated_user.get('_id'))) == new_user.id == user.id
